...
BigInt::BigInteger BigInt::BigInteger::operator*(const BigInt::BigInteger &rhs) const {
    ...
    size_t n = 1;
    while (n < data.size() + rhs.data.size()) {
        n *= 2;
    }
    left_arg.resize(n);
    right_arg.resize(n);
    ...
    result_vec.resize(n);
    for (size_t i = 0; i < n; i++) {
        result_vec[i] = (long long)(round(left_arg[i].real()));
    }
    int rem = 0;
...
BigInt::BigInteger BigInt::BigInteger::getRandOfBitLen(int len) {
    ...
    for (int i = 0; i < len; i++) {
        if (distribution(randomDevice) || i == len - 1) {
            ans += currentPower;
        }
...
void BigInt::fft(std::vector<BigInt::fft_base> &arg, bool invert) {
    ...
    long double angle = 2 * M_PI / n * (invert ? -1 : 1);
...
std::string BigInt::BigInteger::convertToBase2() const {
    std::string answer = "";
    BigInt::BigInteger buff = this->abs();
    if (buff == BigInteger(0)) {
        return "0";
    }
    while (buff != BigInteger(0)) {
        BigInt::BigInteger md = buff % BigInteger(2);
        answer += md.toString();
        buff /= BigInteger(2);
    }
    if (this->sign == MINUS) { answer += '-'; }
    std::reverse(answer.begin(), answer.end());
    return answer;
}

std::string BigInt::BigInteger::convertToBase8() const {
    std::string answer = "";
    BigInt::BigInteger buff = this->abs();
    if (buff == BigInteger(0)) {
        return "0";
    ...
    while (buff != BigInteger(0)) {
        BigInt::BigInteger md = buff % BigInteger(8);
        answer += md.toString();
        buff /= BigInteger(8);
    }
    if (this->sign == MINUS) { answer += '-'; }
...
std::string BigInt::BigInteger::convertToBase10() const {
    return this->toString();
}

std::string BigInt::BigInteger::convertToBase64() const {
    std::string answer = "";
    BigInt::BigInteger buff = this->abs();
    if (buff == BigInteger(0)) {
        return "A";
    ...
    while (buff != BigInteger(0)) {
        BigInt::BigInteger md = buff % BigInteger(64);
        int mdInt = md.toInt();
        if (mdInt < 26) {
            answer += char(mdInt + 'A');
        } else if (mdInt < 52) {
            answer += char(mdInt - 26 + 'a');
        } else if (mdInt < 62) {
            answer += std::to_string(mdInt - 52);
        } else if (mdInt == 62) {
            answer += '+';
        } else {
            answer += '/';
